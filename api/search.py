...
1. **Postcode mode** — geocode an Indonesian postcode, search Google Places for
   businesses within 5 km, and enrich each result with Place Details.
2. **Company mode** — search for a specific company name in Indonesia and return
...
class SearchRequest(BaseModel):
    ...
    Attributes:
        postcode: A 5-digit Indonesian postcode (postcode mode).
...
def _polygon_radius_m(polygon: list[list[float]], centroid: tuple[float, float]) -> float:
    ...
    small distances involved in an Indonesian industrial search area.
    """
    import math
    clat, clng = centroid
...
def _extract_postcode(address: str) -> str:
    """Extract a 5-digit Indonesian postcode from a formatted address.
...
def _search_by_postcode(
    ...
    """Search for businesses near an Indonesian postcode.

    Args:
        gmaps: Authenticated Google Maps client.
        postcode: A 5-digit Indonesian postcode.
    ...
    try:
        geo_results = gmaps.geocode(f"{postcode}, Indonesia")
    except Exception as exc:
    ...
    for keyword in SEARCH_KEYWORDS:
        query = f"{keyword} {postcode} Indonesia"
        try:
