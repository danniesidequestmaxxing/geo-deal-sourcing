...
Indonesia PE Deal Sourcer — identifies manufacturing/industrial acquisition
targets in Indonesia. Web UI + Python backend + standalone CLI.
...
Edit `SEARCH_KEYWORDS` in `api/search.py` (web) or `indonesia_sourcer.py` (CLI).
...
| `indonesia_sourcer.py` | CLI tool — shares `api/_shared/` but has own keywords/delays |
