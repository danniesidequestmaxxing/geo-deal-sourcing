# Indonesia PE Deal Sourcer

Web-based tool for identifying manufacturing and industrial acquisition targets
in Indonesia. Searches Google Places, estimates building footprints via
...
|------|-------|-------------|
| **Postcode** | 5-digit Indonesian postcode | Geocode → 5 km radius text search → filter by postcode |
| **Company** | Company name | Text search across Indonesia |
...
indonesia_sourcer.py        Standalone CLI tool (offline, no web server)
```
...
```json
{
  ...
  "polygon": [[-6.2, 106.8], [-6.2, 106.9], [-6.3, 106.9]]
}
```
...
```json
{
  "places": [
    {
      ...
      "address": "Jl. Industri No. 5, ..., 10110 Jakarta Pusat",
      "phone": "+62 21-5191 1234",
  ...
  "count": 12,
  "postcode": "10110",
  "centroid": { "lat": -6.1856, "lng": 106.845 },
...
```json
{
  "lat": -6.1856,
  "lng": 106.845,
  "viewport": { "northeast": { "lat": -6.184, "lng": 106.847 }, "southwest": { "lat": -6.187, "lng": 106.843 } },
...
{
  "leads": [
    { "name": "PT ABC", "category": "Factory", "address": "...", "website": "https://abc.com" }
  ]
}
```
...
{
  "leads": [
    { "name": "PT ABC", "phone": "+622112345678", "website": "https://abc.com", "business_status": "OPERATIONAL" }
  ]
}
```
...
```bash
# Search by postcodes
python indonesia_sourcer.py --postcodes 10110 40111 60111

# From a file (one postcode per line, # comments allowed)
python indonesia_sourcer.py --file postcodes.txt

# With explicit API key
python indonesia_sourcer.py --postcodes 10110 --api-key YOUR_KEY
```
...
4. **Category default** — Look up the business type in a table of typical
   Indonesian industrial building sizes (e.g. factory → 45,000 sqft).
